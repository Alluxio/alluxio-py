...
def test_worker_net_address_from_worker_info():
    ...
    assert "Provided worker_info is not a valid JSON string" in err.value.args[0]

    non_bytes_worker_info = '{"WorkerNetAddress": {"Host": "localhost"}}'
    with pytest.raises(AttributeError) as err:
        WorkerNetAddress.from_worker_info(non_bytes_worker_info)
    assert "Provided worker_info must be a bytes-like object" in err.value.args[0]
...
def test_worker_net_address_dump_main_info():
    ...
    assert "1234" in result, "rpc port 1234 should be included in the dump info"
    assert (
