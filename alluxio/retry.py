...
logger = logging.getLogger("AlluxioFileSystem")
...
@decorator
async def retry_request(func, retries=1, *args, **kwargs):
    for retry in range(retries):
        try:
            if retry > 0:
                await asyncio.sleep(
                    min(random.random() + 2 ** (retry - 1), 32)
                )
            ...
            if retry == retries - 1:
                logger.exception(
                    f"{func.__name__} out of retries on exception: {e}"
                )
                raise e
            if is_retriable(e):
